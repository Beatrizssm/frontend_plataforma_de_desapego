...
import { Star, ChevronLeft, ChevronRight, User, Search, MessageSquare, Edit2, Trash2 } from "lucide-react";
...
export function ItemDetailsPage() {
  ...
  const handleBuy = async () => {
    ...
    try {
      // Usar a função específica de compra
      await itemService.buyItem(item.id);
      toast.success("Compra realizada com sucesso!");
      // Recarregar item
      const updatedItem = await itemService.getItemById(item.id);
      setItem(updatedItem);
    } catch (error: any) {
      toast.error("Erro ao realizar compra: " + (error.message || "Erro desconhecido"));
    }
  };
  ...
  const getStatusColor = (status: ItemStatus) => {
    const colors = {
      ...
      DOADO_VENDIDO: "bg-gray-100 text-gray-800",
    };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <h1 className="text-2xl font-bold text-[#3A3A3A]">{item.title}</h1>
                {isOwner && (
                  <Select
                    value={item.status}
                    onValueChange={(value) => handleStatusChange(value as ItemStatus)}
                  >
              ...
              <div className="space-y-2 mb-4">
                {isOwner ? (
                  ...
                  <>
                    ...
                    {item.status === "DOADO_VENDIDO" && (
                      <div className="text-center text-sm text-gray-500 py-2">
                        Este item já foi doado/vendido
                      </div>
                    )}
                    <Button
                      onClick={handleChat}
                      ...
                      className="w-full text-sm py-2"
                      disabled={item.status === "DOADO_VENDIDO"}
                    >
                      Chat
                    </Button>
                  </>
